...
import { Scan, UploadCloud, BrainCircuit, CheckCircle2, FileDigit, Cpu, Loader2, Sparkles } from 'lucide-react';
...
export function AiScannerDialog({ onScanned }: { onScanned: () => void }) {
  ...
  const [step, setStep] = useState(0); // 0: upload, 1: scanning, 2: success
  ...
  return (
    <Dialog open={open} onOpenChange={(val) => {
      setOpen(val);
      if (!val) {
        setTimeout(() => { setStep(0); setExtractedData(null); }, 300);
      }
    }}>
      ...
      <DialogContent className="sm:max-w-[550px] bg-zinc-950 border-white/10 text-zinc-200 shadow-2xl">
            ...
            <div className="flex flex-col items-center justify-center py-10 space-y-8">
              ...
              <div className="space-y-3 w-full max-w-[280px]">
                {stepsText.map((text, idx) => (
                  <div key={idx} className="flex items-center gap-3 text-sm animate-in fade-in slide-in-from-bottom-2" style={{ animationDelay: `${idx * 0.8}s`, animationFillMode: 'both' }}>
            ...
            <div className="space-y-6 animate-in zoom-in-95 duration-300">
              ...
              <div className="bg-white/5 border border-white/10 rounded-xl p-4 grid grid-cols-2 gap-4 text-sm">
                ...
                <div className="col-span-2 pt-3 border-t border-white/10 flex justify-between items-center mt-2">
                  ...
                  <span className="text-xl font-bold text-emerald-400">{extractedData.totalAmount} BGN</span>
                </div>
              </div>
              ...
              <div className="bg-indigo-500/10 border border-indigo-500/20 rounded-xl p-3 flex items-start gap-3">
                ...
                <div className="text-xs text-indigo-200">
                  ...
                  Според историята, този разход е за "Компютърно Оборудване". Ще бъде осчетоводен в сметка 601 (Разходи за материали).
                </div>
              </div>
            </div>
          )}
        ...
        <DialogFooter className="border-t border-white/10 pt-4">
          <Button variant="ghost" className="text-zinc-400 hover:text-white hover:bg-white/5" onClick={() => setOpen(false)}>
            Отказ
          </Button>
          {step === 2 && (
            <Button onClick={handleSave} className="bg-emerald-600 hover:bg-emerald-700 text-white shadow-[0_0_15px_rgba(16,185,129,0.3)] border border-emerald-500/50">
              Потвърди и Запиши
            </Button>
          )}
        </DialogFooter>
