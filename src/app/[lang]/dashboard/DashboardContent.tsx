...
import { TrendingUp, ShoppingCart, Wallet, AlertCircle, Inbox, CheckSquare, Clock } from 'lucide-react';
...
function fmt(n: number, lang: string) {
  return n.toLocaleString(lang === 'en' ? 'en-GB' : 'bg-BG', { minimumFractionDigits: 2, maximumFractionDigits: 2 });
}

export default async function DashboardContent({ lang, dict }: { lang: string; dict: any }) {
  const [data, invRes] = await Promise.all([
    getDashboardData(),
    getInvoicesForDashboard(),
  ]);
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="relative overflow-hidden bg-white/5 border border-white/10 rounded-2xl p-5 shadow-lg shadow-violet-500/5 transition-all hover:border-violet-500/30">
          <div className="flex items-center justify-between mb-4">
            <div className="bg-violet-500/20 text-violet-400 rounded-xl p-2.5"><TrendingUp size={18} /></div>
            <span className="text-xs font-medium bg-white/5 px-2.5 py-1 rounded-full text-zinc-400">{t.revenue}</span>
          </div>
          <div className="text-3xl font-bold tracking-tight mb-1 text-white tabular-nums">{fmt(revenue, lang)}</div>
          <div className="text-sm text-zinc-500">€ от продажби{periodLabel ? ` · ${periodLabel}` : ''}</div>
        </div>
        <div className="relative overflow-hidden bg-white/5 border border-white/10 rounded-2xl p-5 shadow-lg shadow-amber-500/5 hover:border-amber-500/30">
          <div className="flex items-center justify-between mb-4">
            <div className="bg-amber-500/20 text-amber-400 rounded-xl p-2.5"><ShoppingCart size={18} /></div>
            <span className="text-xs font-medium bg-white/5 px-2.5 py-1 rounded-full text-zinc-400">{t.expenses}</span>
          </div>
          <div className="text-3xl font-bold tracking-tight mb-1 text-white tabular-nums">{fmt(expenses, lang)}</div>
          <div className="text-sm text-zinc-500">€ разходи за месеца</div>
        </div>
        <div className="relative overflow-hidden bg-white/5 border border-white/10 rounded-2xl p-5 shadow-lg shadow-emerald-500/5 hover:border-emerald-500/30">
          <div className="flex items-center justify-between mb-4">
            <div className={`rounded-xl p-2.5 ${netProfit >= 0 ? 'bg-emerald-500/20 text-emerald-400' : 'bg-red-500/20 text-red-400'}`}><Wallet size={18} /></div>
            <span className="text-xs font-medium bg-white/5 px-2.5 py-1 rounded-full text-zinc-400">Резултат</span>
          </div>
          <div className="text-3xl font-bold tracking-tight mb-1 text-white tabular-nums">{fmt(netProfit, lang)}</div>
          <div className="text-sm text-zinc-500">€ {t.netProfit.toLowerCase()}</div>
        </div>
        <div className="relative overflow-hidden bg-white/5 border border-white/10 rounded-2xl p-5 shadow-lg shadow-rose-500/5 hover:border-rose-500/30">
          <div className="flex items-center justify-between mb-4">
            <div className="bg-rose-500/20 text-rose-400 rounded-xl p-2.5"><AlertCircle size={18} /></div>
            <span className="text-xs font-medium bg-white/5 px-2.5 py-1 rounded-full text-zinc-400">{outstanding.length} бр.</span>
          </div>
          <div className="text-3xl font-bold tracking-tight mb-1 text-white tabular-nums">{fmt(outstandingAmount, lang)}</div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
        <Card className="shadow-sm border-0 bg-white dark:bg-slate-900 border-l-4 border-l-rose-500">
          ...
          <CardContent className="pt-4 space-y-3">
            ...
            <Link href={`/${lang}/dashboard/tasks`} className="mt-4 block w-full text-center bg-muted/50 hover:bg-muted py-2 rounded-lg text-sm font-medium transition-colors">Преглед на всички</Link>
          </CardContent>
        </Card>
        ...
        <Card className="shadow-sm border-0 bg-white dark:bg-slate-900 border-l-4 border-l-indigo-500">
          <CardHeader className="pb-3 border-b border-muted flex flex-row justify-between items-center">
            ...
            {(data?.overviewStats?.inboxOpenItems ?? 0) > 0 && (
              <span className="bg-indigo-100 text-indigo-700 text-xs px-2 py-0.5 rounded-full font-medium">{data?.overviewStats?.inboxOpenItems ?? 0} нови</span>
            )}
          </CardHeader>
          <CardContent className="pt-4">
            {(!data?.aiRecommendations || data.aiRecommendations.length === 0) ? (
              ...
              <div className="space-y-3">
                ...
                <Link href={`/${lang}/dashboard/ai-inbox`} className="mt-4 block w-full text-center bg-muted/50 hover:bg-muted py-2 rounded-lg text-sm font-medium transition-colors">Отваряне на Inbox</Link>
              </div>
            )}
          </CardContent>
        </Card>
        ...
        <Card className="shadow-sm border-0 bg-white dark:bg-slate-900 border-l-4 border-l-emerald-500">
          <CardHeader className="pb-3 border-b border-muted flex flex-row justify-between items-center">
            ...
            {(data?.overviewStats?.approvalsPending ?? 0) > 0 && (
              <span className="bg-emerald-100 text-emerald-700 text-xs px-2 py-0.5 rounded-full font-medium">{data?.overviewStats?.approvalsPending ?? 0} чакащи</span>
            )}
          </CardHeader>
          <CardContent className="pt-4">
            {data?.overviewStats?.approvalsPending === 0 ? (
              ...
              <div className="flex justify-between items-center text-sm py-2">
                ...
                <Link href={`/${lang}/dashboard/approvals`} className="text-emerald-600 hover:underline font-medium">Преглед</Link>
              </div>
            )}
          </CardContent>
        </Card>
