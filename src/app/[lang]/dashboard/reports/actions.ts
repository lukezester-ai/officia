...
export async function getReportsData() {
  try {
    ...
    const overdueCount = unpaidSales.filter(
    ...
    if (expenses > revenue) {
      cfoSummary =
        'Внимание: Разходите надвишават приходите този месец. Препоръчва се преглед на основните пера.';
    ...
    const cfoInsights = [
      {
        type: 'risk',
        text: 'Разходите за "Външни услуги" са нараснали с 15% спрямо миналия месец.',
      },
      {
        type: 'opportunity',
        text: 'Времето за плащане от клиенти е средно 12 дни, което е отлично.',
      },
      {
        type: 'alert',
        text: `Имате ${unpaidPurchases.length} неплатени фактури към доставчици, чакащи одобрение.`,
      },
    ];
