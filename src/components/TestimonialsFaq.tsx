...
const faqs = [
  {
    ...
    answer: "Officia поддържа ДДС дневници и ZIP експорт за НАП като MVP workflow. Директно подаване към НАП, КЕП подпис и финална официална валидация се активират само след реални НАП credentials, сертификат и потвърдена схема."
  },
  {
    question: "Мога ли да импортирам от друга програма?",
    answer: "В момента има CSV обработка за банкови извлечения и подготвена инфраструктура за импорт. Пълният импорт на начални салда, номенклатури и контрагенти е отделен migration workflow и не трябва да се счита за завършен без тест с реални файлове."
  },
  {
    question: "Как работи AI асистентът за документи?",
    answer: "AI асистентът може да извлича данни от изображения на документи чрез vision модел. PDF/OCR pipeline и автоматично осчетоводяване се обработват с човешки преглед при рискови действия."
  },
  {
    question: "Мога ли да използвам Officia за множество фирми?",
    answer: "Системата има tenant модел и данните се пазят по работно пространство. Превключване между много фирми от един профил изисква допълнителен workspace switcher, ако един потребител управлява повече от една фирма."
  },
  {
    question: "Сигурни ли са данните ми?",
    answer: "Достъпът е защитен чрез Clerk и tenant проверки в приложението. Production сигурността зависи и от настройките на хостинг, база данни, RLS, бекъпи и политики за достъп, които трябва да бъдат потвърдени за конкретната среда."
  },
  {
    question: "Има ли безплатен тестов период?",
    answer: "Публичната регистрация може да показва trial оферта, но реалното ограничаване по trial/subscription трябва да е вързано към Stripe и бизнес правилата преди да се обещава като автоматизиран процес."
  }
];
