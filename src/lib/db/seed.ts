...
async function seed() {
  ...
  try {
    ...
    const [acc411, acc503, acc703] = await db.insert(accountPlan).values([
      { tenantId: tenant.id, accountNumber: '411', name: '???????', type: 'asset' },
      { tenantId: tenant.id, accountNumber: '503', name: '????????????? ?????? ? �', type: 'asset' },
      { tenantId: tenant.id, accountNumber: '703', name: '??????? ?? ???????? ?? ??????', type: 'revenue' },
    ]).returning();
    ...
    await db.insert(journalLines).values([
      { journalId: journal.id, accountId: acc503.id, entryType: 'debit', amount: '2400.00', description: '???????????' },
      { journalId: journal.id, accountId: acc411.id, entryType: 'credit', amount: '2400.00', description: '????????? ??????? ?? ??????' },
    ]);
