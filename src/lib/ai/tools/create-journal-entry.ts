...
export const buildCreateJournalEntryTool = (tenantId: string, userId: string) => tool({
  description: "Създава двустранна счетоводна статия (Journal Entry). Използвай го, когато потребителят иска да осчетоводи нещо (напр. 'осчетоводи наем', 'създай контировка'). Високорисково — минава през AI Inbox одобрение.",
  parameters: z.object({
